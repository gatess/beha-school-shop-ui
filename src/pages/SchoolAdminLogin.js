...
const SchoolAdminLogin = (props) => {
  ...
  useEffect(() => {
    if(localStorage.getItem('schoolAdminResult'))
    {
        history.push({
            pathname: '/schooladmin/dashboard'      
          });
    }
  },[]);
  ...
  const handleSubmit = (value) => {
    debugger; 
    setSpinActive(true); 
    fetch("http://beha-schoolshop.eu-central-1.elasticbeanstalk.com/school-admin/login/",{
      method: 'post',
      ...
      .then(
        (result) => {
          ...
          console.log(result);
          if(result.status === 's'){
            localStorage.setItem('schoolId',JSON.stringify(result.schoolId));
            localStorage.setItem('schoolAdminResult',JSON.stringify(result));
           history.push({
            pathname: '/schooladmin/dashboard'
          })
          }else{
    ...
    <div className="gx-login-container">
      <div className="gx-login-content">
        <div className="gx-login-header gx-text-center">
          <h1 className="gx-login-title">Okul Admin Girişi</h1>
        </div>
